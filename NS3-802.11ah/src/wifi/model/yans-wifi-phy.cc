                                            ...
                                            uint8_t packetType, Time rxDuration)
{
  ...
  switch (m_state->GetState ())
    ...
    case YansWifiPhy::IDLE:
      ...
      //std::cout << "Rx Power and Threshold:" << rxPowerW << " " << m_edThresholdW << std::endl;
...
YansWifiPhy::EndReceive (Ptr<Packet> packet, enum WifiPreamble preamble, uint8_t packetType, Ptr<InterferenceHelper::Event> event)
{
  ...
  if (m_plcpSuccess == true)
      ...
      if (m_random->GetValue () > snrPer.per)
        {
          ...
          NS_LOG_DEBUG ("YansWifiPhy::EndReceive, SwitchFromRxEndOk, "  << packet);
        }
      else
        {
          /* failure. */
          NS_LOG_DEBUG ("YansWifiPhy::EndReceive-reason1 ");
          //m_state->SwitchFromRxEndOk (packet, snrPer.snr, event->GetTxVector (), event->GetPreambleType ());
          NotifyRxDrop (packet);
    ...
    {
      //notify rx end
      NS_LOG_DEBUG ("YansWifiPhy::EndReceive-reason2");
      m_state->SwitchFromRxEndError (packet, snrPer.snr);
    }
